...
#define _GL_CALL(x) _glClearError(); x; assert(_glCheckError(#x, __FILE__, __LINE__))
...
namespace
{
    ...
    bool _glCheckError(const char* function, const char* file, int line)
    {
...
int main(int argc, char const *argv[])
{
    ...
    // You need to run the binary from lesson directory '008-error-handling' for these relative paths to work
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // You should really be using glDebugMessageCallback. This is only useful if you need backwards compatibility before 4.3
        _GL_CALL(glDrawElements(GL_TRIANGLES, 6, GL_INT, nullptr));
